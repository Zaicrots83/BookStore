...
export async function postUser(
  ...
  try {
    ...
    console.log("Everthing ok creating the user");
  } catch (error) {
...
export async function deleteUser(id_user: number) {
  try {
    ...
    console.log("Everything OK deleting user");
  } catch (error) {
...
export async function updateUser(
  ...
  user_id: number
    ...
    try {
        await pool.query(`UPDATE users SET user_name = $1, role = $2, phone_number = $3, email = $4, hired_date = $5, password_hash = $6, url_image = $7 WHERE user_id = $8`,
            [User_name,Role,Phone_number,Email,Hired_date,Password_Hash,Url_image,user_id]
        )
    } catch (error) {
